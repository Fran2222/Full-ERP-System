<x-app-layout :assets="['data-table']">
    <div class="container-fluid content-inner mt-n5 py-0">
        ...
        @php
            ...
            abort_unless($canAccess('warehouse.units.view'), 403);

            $canCreateUnit = $canAccess('warehouse.units.create');
            $canEditUnit = $canAccess('warehouse.units.edit');
            $canDeleteUnit = $canAccess('warehouse.units.delete');
        ...
        <div class="card rounded-4 border-0 shadow-sm warehouse-card">
            <div class="card-header bg-white border-0 rounded-top-4 px-4 pt-4 pb-2">
                <div class="d-flex flex-wrap justify-content-between align-items-start gap-3">
                    <div>
                        <h4 class="card-title mb-1 fw-bold">Warehouse Units</h4>
                        <p class="text-secondary mb-0">
                            Manage units of measurement used by warehouse items.
                        </p>
                    </div>

                    @if($canCreateUnit)
                        <a href="{{ route('warehouse.units.create') }}" class="btn btn-primary warehouse-soft-btn">
            ...
            <div class="card-body px-4 pb-4">
                ...
                <div class="table-responsive warehouse-table-wrap">
                    <table id="warehouse-units-table" class="table table-hover align-middle mb-0 warehouse-table w-100">
                        <thead>
                            <tr>
                                <th style="width: 70px;">#</th>
                                <th>Unit Name</th>
                                <th style="width: 220px;">Abbreviation</th>

                                @if($canManageUnitActions)
    ...
    @push('scripts')
        <script>
            $(document).ready(function () {
                ...
                function normalizeWarehouseUnitActionButtons() {
                    $('#warehouse-units-table tbody tr').each(function () {
                        const row = $(this);
                        ...
                        const existingDelete = cell.find('.delete-unit, button[data-url], a[data-url]').first();

                        const editUrl = existingEdit.attr('href') || null;
                        const deleteUrl = existingDelete.data('url') || existingDelete.attr('data-url') || null;
                        const deleteName = existingDelete.data('name') || existingDelete.attr('data-name') || 'this unit';
                        ...
                        if (deleteUrl) {
                            html += `
                                <button type="button"
                                        class="btn btn-sm btn-danger delete-unit wmc-action-btn wmc-action-delete d-inline-flex align-items-center justify-content-center"
                                        data-url="${deleteUrl}"
                ...
                let table = $('#warehouse-units-table').DataTable({
                    processing: false,
                    serverSide: true,
                    responsive: false,
                    autoWidth: false,
                    ...
                    ajax: "{{ route('warehouse.units.index') }}",
                    order: [[0, 'desc']],
                    searchDelay: 250,
                    columns: [
                        {
                    ...
                    drawCallback: function () {
                        normalizeWarehouseUnitActionButtons();
                    },
                    initComplete: function () {
                        normalizeWarehouseUnitActionButtons();

                        const searchInput = $('#warehouse-units-table_filter input');
                        const lengthSelect = $('#warehouse-units-table_length select');
                ...
                $(document).on('click', '.delete-unit', function () {
                    ...
                    let name = $(this).data('name') || 'this unit';
        ...
        <style>
            ...
            #warehouse-units-table.dataTable {
                border: 0 !important;
                margin-bottom: 0 !important;
            }

            #warehouse-units-table.dataTable tbody td.dataTables_empty {
            ...
            #warehouse-units-table tbody tr:has(td.dataTables_empty):hover {
