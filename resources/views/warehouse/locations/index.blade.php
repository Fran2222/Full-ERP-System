<x-app-layout :assets="['data-table']">
    <div class="container-fluid content-inner mt-n5 py-0">
        ...
        @php
            ...
            abort_unless($canAccess('warehouse.locations.view'), 403);

            $canCreateLocation = $canAccess('warehouse.locations.create');
            $canEditLocation = $canAccess('warehouse.locations.edit');
            $canDeleteLocation = $canAccess('warehouse.locations.delete');
            $canManageLocationActions = $canEditLocation || $canDeleteLocation;
        @endphp
        ...
        <div class="card rounded-4 border-0 shadow-sm warehouse-card">
            <div class="card-header bg-white border-0 rounded-top-4 px-4 pt-4 pb-2">
                <div class="d-flex flex-wrap justify-content-between align-items-start gap-3">
                    <div>
                        <h4 class="card-title mb-1 fw-bold">Warehouse Locations</h4>
                        <p class="text-secondary mb-0">
                            Manage warehouse locations and branch-based stock areas.
                        </p>
                    </div>

                    @if($canCreateLocation)
                        <a href="{{ route('warehouse.locations.create') }}" class="btn btn-primary warehouse-soft-btn">
            ...
            <div class="card-body px-4 pb-4">
                ...
                <div class="table-responsive warehouse-table-wrap">
                    <table id="warehouse-locations-table" class="table table-hover align-middle mb-0 warehouse-table w-100">
                        <thead>
                            <tr>
    ...
    @push('scripts')
        <script>
            $(document).ready(function () {
                function normalizeWarehouseLocationActionButtons() {
                    $('#warehouse-locations-table tbody tr').each(function () {
                        const row = $(this);
                        ...
                        const existingDelete = cell.find('.delete-location, button[data-url], a[data-url]').first();

                        const editUrl = existingEdit.attr('href') || null;
                        const deleteUrl = existingDelete.data('url') || existingDelete.attr('data-url') || null;
                        const deleteName = existingDelete.data('name') || existingDelete.attr('data-name') || 'this location';
                        ...
                        if (deleteUrl) {
                            html += `
                                <button type="button"
                                        class="btn btn-sm btn-danger delete-location wmc-action-btn wmc-action-delete d-inline-flex align-items-center justify-content-center"
                                        data-url="${deleteUrl}"
                ...
                let table = $('#warehouse-locations-table').DataTable({
                    processing: false,
                    serverSide: true,
                    responsive: false,
                    autoWidth: false,
                    ...
                    ajax: "{{ route('warehouse.locations.index') }}",
                    order: [[0, 'desc']],
                    searchDelay: 250,
                    columns: [
                        {
                    ...
                    drawCallback: function () {
                        normalizeWarehouseLocationActionButtons();
                    },
                    initComplete: function () {
                        normalizeWarehouseLocationActionButtons();

                        const searchInput = $('#warehouse-locations-table_filter input');
                        const lengthSelect = $('#warehouse-locations-table_length select');
                ...
                $(document).on('click', '.delete-location', function () {
                    ...
                    let name = $(this).data('name') || 'this location';
                            ...
                            success: function (res) {
                                ...
                                if (window.Swal) {
                                    Swal.fire({
                                        icon: 'success',
                                        title: 'Done',
                                        text: res.message || 'Location deleted successfully.',
                                        timer: 1400,
                                        showConfirmButton: false
                                    });
                                } else {
                                    alert(res.message || 'Location deleted successfully.');
                                }
                            },
                            error: function (xhr) {
        ...
        <style>
            ...
            #warehouse-locations-table.dataTable {
                border: 0 !important;
                margin-bottom: 0 !important;
            }

            #warehouse-locations-table.dataTable tbody td.dataTables_empty {
            ...
            #warehouse-locations-table tbody tr:has(td.dataTables_empty):hover {
