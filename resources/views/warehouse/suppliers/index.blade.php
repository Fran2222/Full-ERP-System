<x-app-layout :assets="['data-table']">
    <div class="container-fluid content-inner mt-n5 py-0">
        ...
        @php
            ...
            abort_unless($canAccess('warehouse.suppliers.view'), 403);

            $canCreateSupplier = $canAccess('warehouse.suppliers.create');
            $canEditSupplier = $canAccess('warehouse.suppliers.edit');
            $canDeleteSupplier = $canAccess('warehouse.suppliers.delete');
            $canManageSupplierActions = $canEditSupplier || $canDeleteSupplier;
        @endphp
        ...
        <div class="card warehouse-card">
            <div class="card-header bg-white border-0 rounded-top-4 px-4 pt-4 pb-2">
                <div class="d-flex flex-wrap justify-content-between align-items-start gap-3">
                    <div>
                        <h4 class="card-title mb-1 fw-bold">Suppliers</h4>
                        <p class="text-secondary mb-0">
                            Manage supplier master records used by warehouse and purchasing.
                        </p>
                    </div>

                    @if($canCreateSupplier)
                        <a href="{{ route('warehouse.suppliers.create') }}" class="btn btn-primary warehouse-soft-btn">
                            Add Supplier
                        </a>
                    @endif
                </div>
            ...
            <div class="card-body px-4 pb-4">
                ...
                <div class="table-responsive warehouse-table-wrap">
                    <table id="warehouse-suppliers-table" class="table table-hover align-middle mb-0 warehouse-table w-100">
                        <thead>
                            <tr>
    ...
    @push('scripts')
        <script>
            $(document).ready(function () {
                ...
                function normalizeWarehouseSupplierActionButtons() {
                    $('#warehouse-suppliers-table tbody tr').each(function () {
                        const row = $(this);
                        ...
                        const existingDelete = cell.find('.delete-supplier, button[data-url], a[data-url]').first();

                        const editUrl = existingEdit.attr('href') || null;
                        const deleteUrl = existingDelete.data('url') || existingDelete.attr('data-url') || null;
                        const deleteName = existingDelete.data('name') || existingDelete.attr('data-name') || 'this supplier';
                        ...
                        if (deleteUrl) {
                            html += `
                                <button type="button"
                                        class="btn btn-sm btn-danger delete-supplier wmc-action-btn wmc-action-delete d-inline-flex align-items-center justify-content-center"
                                        data-url="${deleteUrl}"
                ...
                let table = $('#warehouse-suppliers-table').DataTable({
                    processing: false,
                    serverSide: true,
                    responsive: false,
                    autoWidth: false,
                    ...
                    ajax: "{{ route('warehouse.suppliers.index') }}",
                    order: [[0, 'desc']],
                    searchDelay: 250,
                    columns: [
                        {
                    ...
                    language: {
                        search: '',
                        searchPlaceholder: 'Search supplier, contact, phone, email, or address...',
                    ...
                    initComplete: function () {
                        const searchInput = $('#warehouse-suppliers-table_filter input');
                        const lengthSelect = $('#warehouse-suppliers-table_length select');
                    ...
                    drawCallback: function () {
                        normalizeWarehouseSupplierActionButtons();
                    }
                });

                $(document).on('click', '.delete-supplier', function () {
                    let url = $(this).data('url');
                    let name = $(this).data('name') || 'this supplier';
        ...
        <style>
            ...
            #warehouse-suppliers-table.dataTable {
                border: 0 !important;
                margin-bottom: 0 !important;
            }

            #warehouse-suppliers-table.dataTable tbody td.dataTables_empty {
            ...
            #warehouse-suppliers-table tbody tr:has(td.dataTables_empty):hover {
