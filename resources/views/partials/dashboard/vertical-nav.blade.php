@php
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | Module Assignment Compatibility Check
    |--------------------------------------------------------------------------
    | Employee accounts can still have business module assignments such as
    | Sales, Warehouse, and Inventory. Some deployments store those assignments
    | through different table/column names, so this helper checks safely without
    | breaking the sidebar if one source does not exist.
    */
    $normalizeModuleName = function ($value) {
    ...
    $moduleMatches = function ($value, string $module) use ($normalizeModuleName) {
        ...
        foreach (($aliases[$module] ?? [$module]) as $alias) {
            $alias = $normalizeModuleName($alias);
    ...
    $rowEnabled = function ($row) {
        foreach (['is_enabled', 'enabled', 'status', 'module_status'] as $field) {
            if (! isset($row->{$field})) {
                continue;
            }
            ...
            if (in_array($value, ['0', 'false', 'disabled', 'inactive', 'no'], true)) {
                return false;
            }
        }

        foreach (['access_level', 'level', 'role', 'module_role'] as $field) {
            if (! isset($row->{$field})) {
                continue;
            }
            ...
            if (in_array($value, ['', 'disabled', 'none', 'no access'], true)) {
                return false;
            }
        }
    ...
    $userHasAssignedBusinessModule = function (string $module) use ($user, $primaryModule, $moduleMatches, $rowEnabled) {
        if (! $user) {
            return false;
        }

        if ($moduleMatches($primaryModule, $module)) {
            return true;
        }

        foreach ([
            $module . '_access',
            $module . '_access_level',
            $module . '_module',
            'can_access_' . $module,
            'has_' . $module,
            $module . '_enabled',
            ...
            $module . '_module_enabled',
        ] as $field) {
            if (isset($user->{$field})) {
                $value = strtolower((string) $user->{$field});

                if (! in_array($value, ['', '0', 'false', 'disabled', 'none', 'no access'], true)) {
                    return true;
                }
            }
        }

        foreach (['modules', 'module_assignments', 'module_access', 'module_accesses', 'assigned_modules', 'enabled_modules'] as $field) {
            try {
                if (! isset($user->{$field})) {
                    continue;
                }
                ...
                foreach ($items as $key => $item) {
                    if (is_string($key) && $moduleMatches($key, $module)) {
                        if (! in_array(strtolower((string) $item), ['0', 'false', 'disabled', 'none', 'no access'], true)) {
                            return true;
                        }
                    }

                    if (is_string($item) && $moduleMatches($item, $module)) {
                        return true;
                    }
                    ...
                    if (is_array($item)) {
                        ...
                        if (! $rowEnabled($itemObject)) {
                            continue;
                        }

                        foreach (['module', 'module_name', 'module_key', 'module_code', 'name', 'slug', 'module_slug', 'title'] as $moduleField) {
                            if (isset($item[$moduleField]) && $moduleMatches($item[$moduleField], $module)) {
                                return true;
                            }
                        }
                    }
                ...
                // Ignore malformed JSON/casts in sidebar rendering.
            }
        }
        ...
        foreach (['moduleAssignments', 'moduleAccesses', 'assignedModules', 'modules'] as $relation) {
            try {
                ...
                // Ignore unavailable relationship/table in sidebar rendering.
            }
        }
        ...
        try {
            ...
            foreach ($tables as $table) {
                ...
                foreach ($records as $record) {
                    ...
                    if (isset($record->module_id) && Schema::hasTable('modules')) {
                        $moduleRow = DB::table('modules')->where('id', $record->module_id)->first();

                        if ($moduleRow) {
                            foreach (['name', 'module', 'module_name', 'module_key', 'module_code', 'slug', 'title'] as $field) {
                                if (isset($moduleRow->{$field}) && $moduleMatches($moduleRow->{$field}, $module)) {
                                    return true;
                                }
                            }
                        }
            ...
            // Do not let sidebar permission probing break the whole app.
        }
    ...
    if ($isHrManagementAccess || $isEmployeeSelfService || $isDepartmentHeadAccess) {
        // HR/Employee users should still see assigned business modules.
        // Example: employee role + Sales/Warehouse/Inventory Staff access.
        $showInventory = $hasInventoryModule || $isAdminAccess || $canAny([
        ...
        $showWarehouse = $hasWarehouseModule || $isAdminAccess || $canAny([
        ...
        $showPurchasing = $hasPurchasingModule || $isAdminAccess || $canAny([
        ...
        $showAccounting = $hasAccountingModule || $isAdminAccess || $canAny([
            'accounting.view',
            'accounting.create',
            'accounting.edit',
            'accounting.delete',
        ...
        $showProjectManagement = $hasProjectManagementModule || $isAdminAccess || $canAny([
...
@if($isHrEmployeeSidebar)
    ...
    <li class="nav-item">
        <a class="nav-link {{ request()->routeIs('warehouse.*') && ! request()->routeIs('warehouse.inventory') ? 'active' : '' }}" href="{{ $safeRoute('warehouse.dashboard') }}">
            <i class="icon"><svg width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3 10L12 3L21 10V21H3V10Z" fill="currentColor" opacity="0.4"/><path d="M9 21V14H15V21" stroke="currentColor" stroke-width="1.7"/></svg></i>
    ...
    @if($showInventory)
    <li class="nav-item">
        <a class="nav-link {{ request()->routeIs('warehouse.inventory') ? 'active' : '' }}" href="{{ $safeRoute('warehouse.inventory') }}">
            <i class="icon"><svg width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 3L21 8L12 13L3 8L12 3Z" stroke="currentColor" stroke-width="1.7"/><path d="M3 12L12 17L21 12" stroke="currentColor" stroke-width="1.7"/><path d="M3 16L12 21L21 16" stroke="currentColor" stroke-width="1.7"/></svg></i>
    ...
    @if($showPurchasing)
    <li class="nav-item">
        <a class="nav-link {{ request()->routeIs('purchasing.*') ? 'active' : '' }}" href="{{ $safeRoute('purchasing.dashboard') }}">
            <i class="icon"><svg width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4 5H6L8 16H18L21 8H7" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/><circle cx="9" cy="20" r="1" fill="currentColor"/><circle cx="18" cy="20" r="1" fill="currentColor"/></svg></i>
            <span class="item-name">Procurement</span>
        </a>
    </li>
    @endif
    ...
    @if($showSales)
    <li class="nav-item">
        <a class="nav-link {{ request()->routeIs('sales.*') ? 'active' : '' }}" href="{{ $safeRoute('sales.dashboard') }}">
            <i class="icon"><svg width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4 19L9 14L13 17L20 8" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/><path d="M15 8H20V13" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg></i>
