@if(!empty($ajaxTableOnly))
    @php
        ...
        $canVoidReceipt = $canAccess('sales.receipts.void');
        $canManageReceiptActions = $canViewReceipt || $canVoidReceipt;
    @endphp
            ...
            @if($canManageReceiptActions)
                <td class="text-end">
                    <div class="d-inline-flex gap-1">
                        ...
                        @if($canVoidReceipt && strtolower((string) $row->status) !== 'void')
                            <form method="POST"
                                  action="{{ route('sales.sales-receipts.void', $row) }}"
                                  class="sales-receipt-void-form">
                                @csrf
                                @method('PATCH')
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                function confirmVoid() {
                    if (window.Swal) {
                        return Swal.fire({
                            ...
                            text: 'This will keep the receipt record, return stock, and reverse accounting entries when available.',
                    ...
                    return Promise.resolve(confirm('Void this sales receipt? This will return stock and reverse entries when available.'));
                }
                ...
                document.addEventListener('submit', function (event) {
                    const voidForm = event.target.closest('.sales-receipt-void-form');
                    ...
                    confirmVoid().then(function (confirmed) {
                        if (!confirmed) {
                            return;
                        }

                        fetch(voidForm.action, {
                            method: 'POST',
                            headers: {
                            ...
                            body: new FormData(voidForm)
                        })
                            .then(async response => {
                                const data = await response.json().catch(() => ({}));

                                if (!response.ok) {
                                    throw new Error(data.message || 'Unable to void sales receipt.');
                                }

                                return data;
                            })
                            .then(data => {
                                showSwalSuccess(data.message || 'Sales receipt voided and stock returned successfully.');
                                loadSalesReceipts();
                            })
                            .catch(error => {
                                showSwalError(error.message || 'Something went wrong while voiding sales receipt.');
                            });
                    });
                });
            });
