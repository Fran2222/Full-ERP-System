<x-app-layout>
    <div class="container-fluid content-inner mt-n5 py-0 sales-receipt-print-area">
        ...
        @php
            ...
            $canVoidReceipt = $canAccess('sales.receipts.void') && strtolower((string) $salesReceipt->status) !== 'void';
        @endphp
        ...
        <div class="card sales-show-panel">
            <div class="card-header bg-white border-0 rounded-top-4 px-4 pt-4 pb-3">
                <div class="d-flex flex-wrap justify-content-between align-items-start gap-3">
                    ...
                    <div class="d-flex flex-wrap gap-2 no-print">
                        @if($canVoidReceipt)
                            <form method="POST"
                                  action="{{ route('sales.sales-receipts.void', $salesReceipt) }}"
                                  class="d-inline"
                                  onsubmit="return confirm('Void this sales receipt? This will return stock and reverse entries when available.');">
                                @csrf
                                @method('PATCH')
                                <input type="hidden" name="void_reason" value="Voided from receipt page.">
                                <button type="submit" class="btn btn-outline-danger sales-soft-btn">
                                    Void Receipt
                                </button>
                            </form>
                        @endif
            ...
            <div class="card-body px-4 pb-4">
                        ...
                        <div class="sales-info-card h-100">
                            ...
                            <div class="sales-detail-list">
                                ...
                                @if(strtolower((string) $salesReceipt->status) === 'void')
                                    <div>
                                        <span>Voided At</span>
                                        <strong>{{ optional($salesReceipt->voided_at)->format('M d, Y h:i A') ?: '-' }}</strong>
                                    </div>

                                    <div>
                                        <span>Voided By</span>
                                        <strong>{{ $salesReceipt->voidedBy?->name ?? '-' }}</strong>
                                    </div>
                                @endif
                        ...
                        <tbody>
                            @forelse($salesReceipt->items as $line)
                                <tr>
                                    ...
                                    <td>
                                        @if($line->serials && $line->serials->count())
                                            <div class="d-flex flex-wrap gap-1">
                                                @foreach($line->serials as $serial)
                                                    <span class="sales-serial-chip">{{ $serial->serial_number }}</span>
                                                @endforeach
                                            </div>
                                        @else
                ...
                <div class="sales-footer-note mt-4">
                    This receipt is linked to warehouse stock-out movements. Voiding this sales receipt keeps the audit trail, returns the sold quantity back to inventory, and reverses accounting entries when available.
                </div>
