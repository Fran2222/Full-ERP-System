...
class ServiceUnitController extends Controller
{
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($q) use ($search) {
                $q->where('borrow_no', 'ilike', '%' . $search . '%')
                    ...
                    ->orWhere('condition_out', 'ilike', '%' . $search . '%')
                    ->orWhere('condition_in', 'ilike', '%' . $search . '%')
                    ...
                    ->orWhereHas('location', function ($location) use ($search) {
                        $location->where('name', 'ilike', '%' . $search . '%')
                            ->orWhere('location_name', 'ilike', '%' . $search . '%');
                    });
            });
        }

        $borrows = $query->paginate($perPage)->withQueryString();
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($data) {
            ...
            if (! $inventory || (float) $inventory->quantity < 1) {
                throw ValidationException::withMessages([
                    'serial_id' => 'Insufficient available stock for this service unit.',
                ]);
            }
            ...
            $inventory->quantity = (float) $inventory->quantity - 1;
            $inventory->save();
            ...
            StockMovement::create([
                ...
                'reference_type' => $borrow->borrow_no,
                'reference_id' => $borrow->id,
                'remarks' => trim('Service unit borrowed by employee ID ' . $borrow->employee_user_id . ($data['purpose'] ? ' - ' . $data['purpose'] : '')),
    ...
    public function processReturn(Request $request, WarehouseServiceUnitBorrow $serviceUnit)
    {
        ...
        DB::transaction(function () use ($serviceUnit, $data) {
            ...
            $newBorrowStatus = $returnStatus === 'available' ? 'returned' : $returnStatus;

            $serviceUnit->update([
                'status' => $newBorrowStatus,
                'returned_at' => now(),
            ...
            $location = $serviceUnit->location ?: WarehouseLocation::find($serviceUnit->location_id);
            $branchId = $serviceUnit->branch_id ?: $location?->branch_id;

            $inventory = Inventory::firstOrCreate(
                [
                    'item_id' => $serviceUnit->item_id,
                    'branch_id' => $branchId,
                    'location_id' => $serviceUnit->location_id,
                ],
                [
                    'quantity' => 0,
                ]
            ...
            $movementQty = 0;
            $movementType = 'service_unit_return_unavailable';

            if ($returnStatus === 'available') {
                $movementQty = 1;
                $movementType = 'service_unit_return';
            ...
            StockMovement::create([
                ...
                'balance_after' => $inventory->quantity,
                'reference_type' => $serviceUnit->borrow_no,
                'reference_id' => $serviceUnit->id,
                'remarks' => trim('Service unit returned as ' . str_replace('_', ' ', $returnStatus) . ($data['remarks'] ? ' - ' . $data['remarks'] : '')),
