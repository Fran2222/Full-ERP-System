...
class ItemController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::eloquent($items)
                ...
                ->addColumn('display_code', function ($row) {
                    ...
                    return '<span class="fw-semibold text-primary">' . e($code) . '</span>';
                })
                ->addColumn('display_name', function ($row) {
                    $name = $row->name ?: $row->item_name ?: '-';

                    $html = '<div class="fw-semibold text-dark">' . e($name) . '</div>';
                ...
                ->addColumn('action', function ($row) use ($canEditItem, $canDeleteItem) {
                    ...
                    if ($canDeleteItem) {
                        ...
                        $html .= '
                            <button type="button"
                                    class="btn btn-sm btn-outline-danger warehouse-action-btn delete-item"
    ...
    public function store(Request $request)
    {
        ...
        WarehouseItem::create($this->validated($request));
